...
class BertPretrainingCriterion(torch.nn.Module):
    ...
    def forward(self, prediction_scores, seq_relationship_score, masked_lm_labels, next_sentence_labels):
        nvtx.range_push("layer:Masked_LM_Loss")
        masked_lm_loss = self.loss_fn(prediction_scores.view(-1, self.vocab_size), masked_lm_labels.view(-1))
        nvtx.range_pop()
        nvtx.range_push("layer:Next_Sent_Loss")
        next_sentence_loss = self.loss_fn(seq_relationship_score.view(-1, 2), next_sentence_labels.view(-1))
        nvtx.range_pop()
        nvtx.range_push("layer:Total_Loss")
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.fp16:
        ...
        if args.loss_scale == 0:
            ...
            model, optimizer = amp.initialize(model, optimizer, opt_level="O2", loss_scale=2**15, cast_model_outputs=torch.float16)
        else:
