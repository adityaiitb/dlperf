...
class LinearActivation(Module):
    ...
    def forward(self, input):
        if not self.bias is None:
            ...
            nvtx.range_push("layer:Activation")
            x = self.act_fn(x)
            nvtx.range_pop()
            return x
...
class BertLayerNorm(Module):
    ...
    def forward(self, x):
        nvtx.range_push("layer:LayerNorm")
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids):
        ...
        nvtx.range_push("layer:Embedding")
        ...
        nvtx.range_push("layer:Position")
        position_embeddings = self.position_embeddings(position_ids)
        nvtx.range_pop()

        nvtx.range_push("layer:Token")
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
        nvtx.range_pop()

        nvtx.range_push("layer:Total")
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        nvtx.range_push("layer:SelfAttention")

        nvtx.range_push("layer:Q_Proj")
        mixed_query_layer = self.query(hidden_states)
        nvtx.range_pop()

        nvtx.range_push("layer:K_Proj")
        mixed_key_layer = self.key(hidden_states)
        nvtx.range_pop()

        nvtx.range_push("layer:V_Proj")
        ...
        nvtx.range_push("layer:Q.K")
        attention_scores = torch.matmul(query_layer, key_layer)
        nvtx.range_pop()

        nvtx.range_push("layer:Scale")
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        nvtx.range_pop()

        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
        nvtx.range_push("layer:Mask")
        attention_scores = attention_scores + attention_mask
        nvtx.range_pop()

        # Normalize the attention scores to probabilities.
        nvtx.range_push("layer:SoftMax")
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        nvtx.range_push("layer:Attention")
...
class BertLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.id = BertLayer.count
        BertLayer.count += 1

    def forward(self, hidden_states, attention_mask):
        nvtx.range_push("layer:Layer_{}".format(self.id))
        attention_output = self.attention(hidden_states, attention_mask)

        nvtx.range_push("layer:FFN")
...
class BertLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        nvtx.range_push("layer:PredictionHead")
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output):
        nvtx.range_push("layer:PreTrainingHeads")
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
        nvtx.range_pop()
